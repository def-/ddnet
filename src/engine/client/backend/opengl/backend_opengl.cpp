...
{
	size_t AllocSize = pCommand->m_Width * pCommand->m_Height;
	void *pFakeData = malloc(AllocSize);
	void *pFakeDataOutline = malloc(AllocSize);
	mem_zero(pFakeData, AllocSize);
	mem_zero(pFakeDataOutline, AllocSize);
	TextureCreate(pCommand->m_Slot, pCommand->m_Width, pCommand->m_Height, 1, GL_ALPHA, GL_ALPHA, CCommandBuffer::TEXFLAG_NOMIPMAPS, pFakeData);
	TextureCreate(pCommand->m_SlotOutline, pCommand->m_Width, pCommand->m_Height, 1, GL_ALPHA, GL_ALPHA, CCommandBuffer::TEXFLAG_NOMIPMAPS, pFakeDataOutline);
}
