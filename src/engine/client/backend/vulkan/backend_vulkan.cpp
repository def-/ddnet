// TODO
#if defined(CONF_BACKEND_VULKAN) || 1
...
class CCommandProcessorFragment_Vulkan : public CCommandProcessorFragment_GLBase
{
	...
	struct SMemoryHeap
	{
		...
		typedef std::multiset<SMemoryHeapQueueElement, std::greater<SMemoryHeapQueueElement>> TMemoryHeapQueue;
		...
		// make sure every allocation aligns the remaining size correctly (especially textures require quite alot of alignment)
		size_t m_RequiredAlignment;

		void Init(size_t Size, size_t Offset, size_t RequiredAlignment)
		{
			m_RequiredAlignment = RequiredAlignment;
			...
			QueueEl.m_Offset = Offset;
			QueueEl.m_pElementInHeap = &m_Root;
			m_Root.m_InQueue = m_Elements.insert(QueueEl);
		}

		bool Allocate(size_t AllocSize, SMemoryHeapQueueElement &AllocatedMemory)
		{
			if(m_Elements.empty())
			{
				return false;
			...
			else
			{
				...
				size_t AllocSizeToAlign = AllocSize % m_RequiredAlignment;
				if(AllocSizeToAlign != 0)
					AllocSizeToAlign = m_RequiredAlignment - AllocSizeToAlign;
				size_t RealAllocSize = AllocSize + AllocSizeToAlign;
				...
				else
				{
					...
					TopEl.m_pElementInHeap->m_pLeft = std::unique_ptr<SMemoryHeapElement>(new SMemoryHeapElement{});
					TopEl.m_pElementInHeap->m_pLeft->m_AllocationSize = RealAllocSize;
					TopEl.m_pElementInHeap->m_pLeft->m_Offset = TopEl.m_Offset;
					...
					if(RealAllocSize < TopEl.m_AllocationSize)
					{
						...
						RemainingEl.m_Offset = TopEl.m_Offset + RealAllocSize;
						RemainingEl.m_AllocationSize = TopEl.m_AllocationSize - RealAllocSize;

						TopEl.m_pElementInHeap->m_pRight = std::unique_ptr<SMemoryHeapElement>(new SMemoryHeapElement{});
						TopEl.m_pElementInHeap->m_pRight->m_AllocationSize = RemainingEl.m_AllocationSize;
						TopEl.m_pElementInHeap->m_pRight->m_Offset = RemainingEl.m_Offset;
					...
					AllocatedMemory.m_Offset = TopEl.m_Offset;
					return true;
				}
			}
		}
		...
		void Free(SMemoryHeapQueueElement &AllocatedMemory)
		{
			...
			while(ContinueFree)
			{
				...
				if((pThisParent != nullptr && pOtherHeapObj == nullptr) || (pOtherHeapObj != nullptr && !pOtherHeapObj->m_InUse))
				{
					...
					ParentEl.m_Offset = pThisParent->m_Offset;
	...
	{
		SDeviceDescriptorSet m_UniformSet;
	...
	{
		std::vector<std::vector<TName>> m_BufferObjectsOfFrame;
		std::vector<std::vector<VkMappedMemoryRange>> m_BufferObjectsOfFrameRangeData;
		std::vector<size_t> m_CurrentUsedCount;
...
private:
	...
	SDeviceDescriptorPools m_UniformBufferDescrPool;

	VkSwapchainKHR m_VKSwapChain = VK_NULL_HANDLE;
	std::vector<VkImage> m_SwapChainImages;
	uint32_t m_SwapChainImageCount = 0;

	SStreamMemory<SFrameBuffers> m_StreamedVertexBuffer;
	SStreamMemory<SFrameUniformBuffers> m_StreamedUniformBuffer;
...
protected:
	...
	{
		dbg_msg("vulkan", "error: %s", pErr);
		dbg_assert(false, pErr);
	}

	void SetWarning(const char *pErr)
	{
		dbg_msg("vulkan", "warning: %s", pErr);
	}

	bool CheckVulkanCriticalError(VkResult CallResult)
	{
		bool CriticalError = false;
		switch(CallResult)
		{
		case VK_ERROR_OUT_OF_HOST_MEMORY:
			dbg_msg("vulkan", "host ran out of memory");
			CriticalError = true;
			break;
		case VK_ERROR_OUT_OF_DEVICE_MEMORY:
			dbg_msg("vulkan", "device ran out of memory");
			CriticalError = true;
			break;
		case VK_ERROR_DEVICE_LOST:
			dbg_msg("vulkan", "device lost");
			CriticalError = true;
			break;
		...
		case VK_ERROR_INCOMPATIBLE_DRIVER:
			dbg_msg("vulkan", "no compatible driver found. Vulkan 1.1 is required.");
			CriticalError = true;
			return false;
		case VK_ERROR_INITIALIZATION_FAILED:
			dbg_msg("vulkan", "initialization failed for unknown reason.");
			CriticalError = true;
			return false;
		...
		case VK_SUCCESS:
			break;
		default:
			dbg_msg("vulkan", "unknown error %u", (uint32_t)CallResult);
			CriticalError = true;
			break;
		}
	...
	************************/
	...
	template<size_t ID,
		ssize_t MemoryBlockSize, size_t BlockCount,
	...
	SMemoryBlock<ID> GetBufferBlockImpl(SMemoryBlockCache<ID> &MemoryCache, VkBufferUsageFlags BufferUsage, VkMemoryPropertyFlags BufferProperties, const void *pBufferData, VkDeviceSize RequiredSize, VkDeviceSize TargetAlignment)
	{
		...
		// calculate the alignment
		size_t AllocSizeToAlign = RequiredSize % TargetAlignment;
		if(AllocSizeToAlign != 0)
			AllocSizeToAlign = TargetAlignment - AllocSizeToAlign;
		size_t RealAllocSize = RequiredSize + AllocSizeToAlign;
		...
		auto &&CreateCacheBlock = [&]() {
			...
			{
				auto *pHeap = Heaps[i];
				if(pHeap->m_Heap.Allocate(RealAllocSize, AllocatedMem))
				{
			...
			if(!FoundAllocation)
			{
				...
				Heaps.back()->m_Heap.Init(MemoryBlockSize * BlockCount, 0, TargetAlignment);
				if(!Heaps.back()->m_Heap.Allocate(RealAllocSize, AllocatedMem))
				{
			...
			if(RequiresMapping)
				RetBlock.m_pMappedBuffer = ((uint8_t *)pCacheHeap->m_pMappedBuffer) + AllocatedMem.m_Offset;
			else
			...
			RetBlock.m_UsedSize = RealAllocSize;

			if(RequiresMapping)
				mem_copy(RetBlock.m_pMappedBuffer, pBufferData, RequiredSize);
		};

		if(RealAllocSize < (VkDeviceSize)MemoryBlockSize)
		{
			CreateCacheBlock();
		}
		else
		{
			VkBuffer TmpBuffer;
			SDeviceMemoryBlock TmpBufferMemory;
			GetBufferImpl(RealAllocSize, RequiresMapping ? MEMORY_BLOCK_USAGE_STAGING : MEMORY_BLOCK_USAGE_BUFFER, TmpBuffer, TmpBufferMemory, BufferUsage, BufferProperties);
			...
			RetBlock.m_HeapData.m_Offset = 0;
			RetBlock.m_HeapData.m_AllocationSize = RealAllocSize;
			RetBlock.m_UsedSize = RealAllocSize;
		}
	...
	{
		return GetBufferBlockImpl<s_StagingBufferCacheID, 8 * 1024 * 1024, 3, true>(m_StagingBufferCache, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, pBufferData, RequiredSize, 16);
	}

	SMemoryBlock<s_StagingBufferImageCacheID> GetStagingBufferImage(const void *pBufferData, VkDeviceSize RequiredSize)
	{
		return GetBufferBlockImpl<s_StagingBufferImageCacheID, 8 * 1024 * 1024, 3, true>(m_StagingBufferCacheImage, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, pBufferData, RequiredSize, maximum<VkDeviceSize>(m_OptimalImageCopyMemAlignment, 16));
	}
	...
	void FreeStagingImageMemBlock(SMemoryBlock<s_StagingBufferImageCacheID> &Block)
	{
		if(!Block.m_IsCached)
		{
			m_FrameDelayedBufferCleanup[m_CurImageIndex].push_back({Block.m_Buffer, Block.m_BufferMem});
	...
	static constexpr ssize_t s_1024x1024ImgSize = (1024 * 1024 * 4) * 2;
	...
	SMemoryBlock<ID> GetImageMemoryBlockImpl(SMemoryBlockCache<ID> &MemoryCache, VkMemoryPropertyFlags BufferProperties, VkDeviceSize RequiredSize, bool IsRGBAAnd2D)
	{
		...
		// calculate the alignment
		size_t AllocSizeToAlign = RequiredSize % m_DummyImageMemRequirements.alignment;
		if(AllocSizeToAlign != 0)
			AllocSizeToAlign = m_DummyImageMemRequirements.alignment - AllocSizeToAlign;
		size_t RealAllocSize = RequiredSize + AllocSizeToAlign;
		...
		auto &&CreateCacheBlock = [&]() {
			...
			if(!FoundAllocation)
			{
				...
				pCacheHeap = pNewHeap;
				Heaps.emplace_back(pNewHeap);
				Heaps.back()->m_Heap.Init(MemoryBlockSize * BlockCount, 0, m_DummyImageMemRequirements.alignment);
				if(!Heaps.back()->m_Heap.Allocate(RealAllocSize, AllocatedMem))
				{
		...
		if(IsRGBAAnd2D && RealAllocSize < (VkDeviceSize)MemoryBlockSize)
		{
			CreateCacheBlock();
		}
		else
		{
			SDeviceMemoryBlock TmpBufferMemory;
			GetImageMemoryImpl(RealAllocSize, TmpBufferMemory, BufferProperties);
	...
	{
		return GetImageMemoryBlockImpl<s_ImageBufferCacheID, s_1024x1024ImgSize, 2>(m_ImageBufferCache, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, RequiredSize, Is2DAndRGBA);
	}
	...
	void ClearFrameData(size_t FrameImageIndex)
	{
		...
		{
			CleanBufferPair(FrameImageIndex, BufferPair.first, BufferPair.second);
		}
		...
		m_ImageBufferCache.Cleanup(FrameImageIndex);
	}
	...
	void ShrinkUnusedCaches()
	{
		...
		FreeedMemory = 0;
		FreeedMemory += m_ImageBufferCache.Shrink(m_VKDevice);
		if(FreeedMemory > 0)
		{
	...
	************************/
	...
	{
		// streamed vertices
		UploadStreamedBuffer(m_StreamedVertexBuffer);
		// now the buffer objects
		UploadStreamedBuffer(m_StreamedUniformBuffer);
		...
		VkSemaphore aWaitSemaphores[] = {WaitSemaphore};
		...
		SubmitInfo.pWaitDstStageMask = aWaitStages;
		...
		{
			bool IsCritical = CheckVulkanCriticalError(QueueSubmitRes);
			if(IsCritical)
		...
		VkSwapchainKHR aSwapChains[] = {m_VKSwapChain};
		...
		{
			bool CriticalError = CheckVulkanCriticalError(QueuePresentRes);
			if(CriticalError)
				SetError("Presenting graphics queue failed.");
		}
	...
	void PrepareFrame()
	{
		...
		if(AcqResult != VK_SUCCESS)
		{
			...
			else if(AcqResult != VK_SUBOPTIMAL_KHR)
			{
				...
				bool IsCritical = CheckVulkanCriticalError(AcqResult);
				if(IsCritical)
					SetError("Acquiring next image failed.");
				else if(AcqResult == VK_ERROR_SURFACE_LOST_KHR)
				{
					m_RenderingPause = true;
					return;
				}
			}
		}
		...
		// clear frame
		vkResetCommandBuffer(GetCurrentCommandBuffer(), VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
	...
	************************/
	...
	void UpdateTexture(size_t TextureSlot, VkFormat Format, void *&pData, ssize_t XOff, ssize_t YOff, size_t Width, size_t Height, size_t ColorChannelCount)
	{
		...
		CopyBufferToImage(StagingBuffer.m_Buffer, StagingBuffer.m_HeapData.m_Offset, Tex.m_Img, XOff, YOff, Width, Height, 1);
		...
		FreeStagingImageMemBlock(StagingBuffer);
	}
	...
	************************/
	...
	void CreateBufferObject(size_t BufferIndex, const void *pUploadData, VkDeviceSize BufferDataSize, bool IsOneFrameBuffer)
	{
		...
		if(!IsOneFrameBuffer)
		{
			...
			BufferOffset = Mem.m_HeapData.m_Offset;

			MemoryBarrier(VertexBuffer, Mem.m_HeapData.m_Offset, BufferDataSize, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, true);
			CopyBuffer(StagingBuffer.m_Buffer, VertexBuffer, StagingBuffer.m_HeapData.m_Offset, Mem.m_HeapData.m_Offset, BufferDataSize);
			MemoryBarrier(VertexBuffer, Mem.m_HeapData.m_Offset, BufferDataSize, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, false);
	...
	{
		IsTextured = State.m_Texture != -1;
		AddressModeIndex = GetAddressModeIndex(State);
		BlendModeIndex = GetBlendModeIndex(State);
		DynamicIndex = GetDynamicModeIndex(State);
	}

	void BindPipeline(VkCommandBuffer &CommandBuffer, VkPipeline &BindingPipe, const CCommandBuffer::SState &State)
	{
		...
		if(DynamicStateIndex == VULKAN_BACKEND_CLIP_MODE_DYNAMIC_SCISSOR_AND_VIEWPORT)
		{
			...
			int32_t ScissorY = (int32_t)m_VKSwapImgExtent.height - ((int32_t)State.m_ClipY + (int32_t)State.m_ClipH);
	...
	void RenderTileLayer(const CCommandBuffer::SState &State, size_t BufferContainerIndex, int Type, const GL_SColorf &Color, const vec2 &Dir, const vec2 &Off, int32_t JumpIndex, size_t IndicesDrawNum, char *const *pIndicesOffsets, const unsigned int *pDrawCount, size_t InstanceCount)
	{
		...
		BindPipeline(CommandBuffer, PipeLine, State);
		...
		if(IsTextured)
		{
			...
			vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PipeLayout, 0, 1, &DescrSet.m_Descriptor, 0, nullptr);
		}
...
public:
	...
	************************/
	...
	bool CreateVulkanInstance(const std::vector<std::string> &VKLayers, const std::vector<std::string> &VKExtensions, bool TryDebugExtensions)
	{
		...
		bool IsCritical = CheckVulkanCriticalError(Res);
		if(IsCritical)
		{
			SetError("Creating instance failed.");
			return false;
		}
	...
	bool SelectGPU(char *pRendererName, char *pVendorName, char *pVersionName)
	{
		...
		for(auto &CurDevice : DeviceList)
		{
			...
			if((AutoGPUIndex == 0 && DeviceProp.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) && (DevAPIMajor > 1 || (DevAPIMajor == 1 && DevAPIMinor >= 1)))
			{
			...
			if(((IsAutoGPU && FoundDeviceIndex == 0 && DeviceProp.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) || str_comp(DeviceProp.deviceName, g_Config.m_GfxGPUName) == 0) && (DevAPIMajor > 1 || (DevAPIMajor == 1 && DevAPIMinor >= 1)))
			{
...

		{
			...
			if(IsVerbose())
			{
				...
				dbg_msg("vulkan", "device prop: min uniform align: %u", m_MinUniformAlign);
			}
		}
	...
	bool GetPresentationMode(VkPresentModeKHR &VKIOMode)
	{
		...
		VKIOMode = g_Config.m_GfxVsync ? VK_PRESENT_MODE_MAILBOX_KHR : VK_PRESENT_MODE_FIFO_RELAXED_KHR;
		for(auto &Mode : PresentModeList)
		{
			if(Mode == VKIOMode)
				return true;
	...
	uint32_t GetNumberOfSwapImages(const VkSurfaceCapabilitiesKHR &VKCapabilities)
	{
		...
		ImgNumber = VKCapabilities.minImageCount;
#endif
		if(IsVerbose())
		{
	...
	bool CreateSwapChain()
	{
		...
		bool IsCritialError = CheckVulkanCriticalError(SwapchainCreateRes);
		if(IsCritialError)
		{
	...
	bool CreateFramebuffers()
	{
		...
		for(size_t i = 0; i < m_SwapChainImageCount; i++)
		{
			VkImageView aAttachments[] = {m_SwapChainImageViewList[i]};
	...
	template<size_t ArraySize, size_t DescrArraySize, size_t PushArraySize>
		...
		EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode, bool IsLinePrim = false)
	{
		...
		PipelineLayoutInfo.setLayoutCount = HasSampler ? aSetLayouts.size() : 0;
		PipelineLayoutInfo.pSetLayouts = HasSampler && !aSetLayouts.empty() ? aSetLayouts.data() : nullptr;
	...
	bool CreateStandardGraphicsPipelineImpl(const char *pVertName, const char *pFragName, SPipelineContainer &PipeContainer, EVulkanBackendTextureModes TexMode, EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode, bool IsLinePrim)
	{
		...
		return CreateGraphicsPipeline(pVertName, pFragName, PipeContainer, sizeof(float) * (2 + 2) + sizeof(uint8_t) * 4, aAttributeDescriptions, aSetLayouts, aPushConstants, TexMode, BlendMode, DynamicMode, IsLinePrim);
	}
	...
	bool CreateStandard3DGraphicsPipelineImpl(const char *pVertName, const char *pFragName, SPipelineContainer &PipeContainer, EVulkanBackendTextureModes TexMode, EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode)
	{
		...
		return CreateGraphicsPipeline(pVertName, pFragName, PipeContainer, sizeof(float) * 2 + sizeof(uint8_t) * 4 + sizeof(float) * 3, aAttributeDescriptions, aSetLayouts, aPushConstants, TexMode, BlendMode, DynamicMode);
	}
	...
	bool CreateTextGraphicsPipelineImpl(const char *pVertName, const char *pFragName, SPipelineContainer &PipeContainer, EVulkanBackendTextureModes TexMode, EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode)
	{
		...
		return CreateGraphicsPipeline(pVertName, pFragName, PipeContainer, sizeof(float) * (2 + 2) + sizeof(uint8_t) * 4, aAttributeDescriptions, aSetLayouts, aPushConstants, TexMode, BlendMode, DynamicMode);
	}
	...
	bool CreateTileGraphicsPipelineImpl(const char *pVertName, const char *pFragName, int Type, SPipelineContainer &PipeContainer, EVulkanBackendTextureModes TexMode, EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode)
	{
		...
		return CreateGraphicsPipeline(pVertName, pFragName, PipeContainer, HasSampler ? (sizeof(float) * (2 + 3)) : (sizeof(float) * 2), aAttributeDescriptions, aSetLayouts, aPushConstants, TexMode, BlendMode, DynamicMode);
	}
	...
	{
		vkDestroyDescriptorSetLayout(m_VKDevice, m_UniformDescriptorSetLayout, nullptr);
	}

	bool CreateUniformDescriptorSets(SDeviceDescriptorSet *pSets, size_t SetCount, VkBuffer BindBuffer, size_t SingleBufferInstanceSize, VkDeviceSize MemoryOffset)
	{
		GetDescriptorPoolForAlloc(m_UniformBufferDescrPool, pSets, SetCount);
	...
	bool CreateSpriteMultiGraphicsPipelineImpl(const char *pVertName, const char *pFragName, SPipelineContainer &PipeContainer, EVulkanBackendTextureModes TexMode, EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode)
	{
		...
		aSetLayouts[0] = m_StandardTexturedDescriptorSetLayout;
		aSetLayouts[1] = m_UniformDescriptorSetLayout;
	...
	{
		m_DrawCommandBuffers.resize(m_SwapChainImageCount);
		...
		if(vkAllocateCommandBuffers(m_VKDevice, &AllocInfo, m_DrawCommandBuffers.data()) != VK_SUCCESS)
		{
	...
	void DestroyCommandBuffer()
	{
		...
		vkFreeCommandBuffers(m_VKDevice, m_CommandPool, static_cast<uint32_t>(m_DrawCommandBuffers.size()), m_DrawCommandBuffers.data());
	...
	{
		CleanBufferPair(ImageIndex, Buffer.m_Buffer, Buffer.m_BufferMem);
		auto &DescrSet = Buffer.m_UniformSet;
	...
	void CleanupVulkan()
	{
		...
		m_StreamedVertexBuffer.Destroy([&](size_t ImageIndex, SFrameBuffers &Buffer) { DestroyBufferOfFrame(ImageIndex, Buffer); });
		m_StreamedUniformBuffer.Destroy([&](size_t ImageIndex, SFrameUniformBuffers &Buffer) { DestroyUniBufferOfFrame(ImageIndex, Buffer); });
		...
		m_ImageBufferCache.DestroyFrameData(m_SwapChainImageCount);
		...
		if(IsLastCleanup)
		{
			...
			m_ImageBufferCache.Destroy(m_VKDevice);
		...
		if(IsLastCleanup)
		{
			DestroyUniformDescriptorSetLayout();
		...
		DestroySwapChain(IsLastCleanup);
	...
	int RecreateSwapChain()
	{
		...
		if(m_SwapchainCreated)
			CleanupVulkan<false>();
		if(!m_SwapchainCreated)
			return InitVulkan<false>(m_pStorage);
	...
	bool CreateBuffer(VkDeviceSize BufferSize, EMemoryBlockUsage MemUsage, VkBufferUsageFlags BufferUsage, VkMemoryPropertyFlags BufferProperties, VkBuffer &VKBuffer, SDeviceMemoryBlock &VKBufferMemory)
	{
	...
	bool CreateDescriptorPools()
	{
		...
		m_UniformBufferDescrPool.m_IsUniformPool = true;
		m_UniformBufferDescrPool.m_DefaultAllocSize = 512;
		...
		Ret |= AllocateDescriptorPool(m_UniformBufferDescrPool, 64);
	...
	void DestroyDescriptorPools()
	{
		...
		for(auto &DescrPool : m_TextTextureDescrPool.m_Pools)
			vkDestroyDescriptorPool(m_VKDevice, DescrPool.m_Pool, nullptr);
		for(auto &DescrPool : m_UniformBufferDescrPool.m_Pools)
			vkDestroyDescriptorPool(m_VKDevice, DescrPool.m_Pool, nullptr);
	}
	...
	int InitVulkan(IStorage *m_pStorage)
	{
		...
		m_StreamedVertexBuffer.Init(m_SwapChainImageCount);
		m_StreamedUniformBuffer.Init(m_SwapChainImageCount);
		...
		m_ImageBufferCache.Init(m_SwapChainImageCount);
		...
		if(IsFirstInitialization)
		{
			...
			{
				dbg_msg("vulkan", "failed to create dummy image!");
			}
			...
			if(IsVerbose())
			{
				dbg_msg("vulkan", "alignment for 1024x1024 image is %u", (uint32_t)m_DummyImageMemRequirements.alignment);
			}
	...
	{
		return m_DrawCommandBuffers[m_CurImageIndex];
	}
	...
	void CreateStreamBuffer(TStreamMemName *&pBufferMem, TNewMemFunc &&NewMemFunc, SStreamMemory<TStreamMemName> &StreamUniformBuffer, VkBufferUsageFlagBits Usage, VkBuffer &NewBuffer, SDeviceMemoryBlock &NewBufferMem, size_t &BufferOffset, const void *pData, size_t DataSize)
	{
		...
		if(BufferMem.m_Mem == VK_NULL_HANDLE)
		{
			...
			auto &NewBuffer = StreamUniformBuffer.GetBuffers(m_CurImageIndex)[NewBufferIndex];

			Buffer = StreamBuffer;
			BufferMem = StreamBufferMemory;

			pBufferMem = &NewBuffer;
			pMem = (uint8_t *)NewBuffer.m_pMappedBufferData;
			Offset = NewBuffer.m_OffsetInBuffer;
			NewBuffer.m_UsedSize += DataSize;
	...
	void CreateStreamVertexBuffer(VkBuffer &NewBuffer, SDeviceMemoryBlock &NewBufferMem, size_t &BufferOffset, const void *pData, size_t DataSize)
	{
		...
		CreateStreamBuffer<SFrameBuffers, GL_SVertexTex3DStream, CCommandBuffer::MAX_VERTICES, 1, false>(
			pStreamBuffer, [](SFrameBuffers &, VkBuffer, VkDeviceSize) {}, m_StreamedVertexBuffer, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, NewBuffer, NewBufferMem, BufferOffset, pData, DataSize);
	}

	template<typename TName, size_t InstanceMaxParticleCount, size_t MaxInstances>
	void GetUniformBufferObjectImpl(SStreamMemory<SFrameUniformBuffers> &StreamUniformBuffer, SDeviceDescriptorSet &DescrSet, const void *pData, size_t DataSize)
	{
		VkBuffer NewBuffer;
		...
		CreateStreamBuffer<SFrameUniformBuffers, TName, InstanceMaxParticleCount, MaxInstances, true>(
			pMem,
			[this](SFrameBuffers &Mem, VkBuffer Buffer, VkDeviceSize MemOffset) { CreateUniformDescriptorSets(&((SFrameUniformBuffers *)(&Mem))->m_UniformSet, 1, Buffer, InstanceMaxParticleCount * sizeof(TName), MemOffset); },
			StreamUniformBuffer, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, NewBuffer, NewBufferMem, BufferOffset, pData, DataSize);

		DescrSet = pMem->m_UniformSet;
	}

	void GetUniformBufferObject(SDeviceDescriptorSet &DescrSet, size_t ParticleCount, const void *pData, size_t DataSize)
	{
		GetUniformBufferObjectImpl<IGraphics::SRenderSpriteInfo, 512, 128>(m_StreamedUniformBuffer, DescrSet, pData, DataSize);
	}
	...
	bool CreateIndexBuffer(void *pData, size_t DataSize, VkBuffer &Buffer, SDeviceMemoryBlock &Memory)
	{
		...
		CopyBuffer(StagingBuffer.m_Buffer, VertexBuffer, StagingBuffer.m_HeapData.m_Offset, 0, BufferDataSize);
	...
	bool CreateTextureImage(size_t ImageIndex, VkImage &NewImage, SMemoryBlock<s_ImageBufferCacheID> &NewImgMem, const void *pData, VkFormat Format, size_t Width, size_t Height, size_t Depth, size_t PixelSize, size_t MipMapLevelCount)
	{
		...
		CopyBufferToImage(StagingBuffer.m_Buffer, StagingBuffer.m_HeapData.m_Offset, NewImage, 0, 0, static_cast<uint32_t>(Width), static_cast<uint32_t>(Height), Depth);
	...
	void CreateImage(uint32_t Width, uint32_t Height, uint32_t Depth, size_t MipMapLevelCount, VkFormat Format, VkImageTiling Tiling, VkImage &Image, SMemoryBlock<s_ImageBufferCacheID> &ImageMemory)
	{
		...
		vkBindImageMemory(m_VKDevice, Image, ImageMem.m_BufferMem.m_Mem, ImageMem.m_HeapData.m_Offset);
	}
	...
	************************/
	...
	bool RunCommand(const CCommandBuffer::SCommand *pBaseCommand) override
	{
		...
		case CCommandBuffer::CMD_WINDOW_CREATE_NTF:
			Cmd_WindowCreateNtf(static_cast<const CCommandBuffer::SCommand_WindowCreateNtf *>(pBaseCommand));
			return false;
		case CCommandBuffer::CMD_WINDOW_DESTROY_NTF:
			Cmd_WindowDestroyNtf(static_cast<const CCommandBuffer::SCommand_WindowDestroyNtf *>(pBaseCommand));
			return false;
	...
	void Cmd_TextTextures_Create(const CCommandBuffer::SCommand_TextTextures_Create *pCommand)
	{
		...
		CreateTextureCMD(SlotOutline, Width, Height, 1, VK_FORMAT_R8_UNORM, VK_FORMAT_R8_UNORM, CCommandBuffer::TEXFLAG_NOMIPMAPS, pTmpData);
	...
	void RenderStandard(const CCommandBuffer::SState &State, int PrimType, const TName *pVertices, int PrimitiveCount)
	{
		std::array<float, (size_t)4 * 2> m;
		GetStateMatrix(State, m);
		...
		size_t BufferOff = 0;
		CreateStreamVertexBuffer(VKBuffer, VKBufferMem, BufferOff, pVertices, VertPerPrim * sizeof(TName) * PrimitiveCount);
		...
		if(IsTextured)
		{
			auto &DescrSet = Is3DTextured ? m_Textures[State.m_Texture].m_VKStandard3DTexturedDescrSet : m_Textures[State.m_Texture].m_aVKStandardTexturedDescrSets[AddressModeIndex];
			vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PipeLayout, 0, 1, &DescrSet.m_Descriptor, 0, nullptr);
		}
	...
	{
		RenderStandard<CCommandBuffer::SVertex, false>(pCommand->m_State, pCommand->m_PrimType, pCommand->m_pVertices, pCommand->m_PrimCount);
	}
	...
	{
		RenderStandard<CCommandBuffer::SVertexTex3DStream, true>(pCommand->m_State, pCommand->m_PrimType, pCommand->m_pVertices, pCommand->m_PrimCount);
	}
	...
	void Cmd_Update_Viewport(const CCommandBuffer::SCommand_Update_Viewport *pCommand)
	{
		...
		{
			if(pCommand->m_X != 0 || pCommand->m_Y != 0 || (uint32_t)pCommand->m_Width != m_VKSwapImgExtent.width || (uint32_t)pCommand->m_Height != m_VKSwapImgExtent.height)
			{
				m_HasDynamicViewport = true;

				// convert viewport from OGL to vulkan
				int32_t ViewportY = (int32_t)m_VKSwapImgExtent.height - ((int32_t)pCommand->m_Y + (int32_t)pCommand->m_Height);
	...
	void Cmd_UpdateBufferObject(const CCommandBuffer::SCommand_UpdateBufferObject *pCommand)
	{
		...
		MemoryBarrier(VertexBuffer, Offset + MemBlock.m_HeapData.m_Offset, DataSize, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, true);
		CopyBuffer(StagingBuffer.m_Buffer, VertexBuffer, StagingBuffer.m_HeapData.m_Offset, Offset + MemBlock.m_HeapData.m_Offset, DataSize);
		MemoryBarrier(VertexBuffer, Offset + MemBlock.m_HeapData.m_Offset, DataSize, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, false);
	...
	void Cmd_CopyBufferObject(const CCommandBuffer::SCommand_CopyBufferObject *pCommand)
	{
		...
		VkDeviceSize ReadOffset = (VkDeviceSize)pCommand->m_pReadOffset + ReadMemBlock.m_HeapData.m_Offset;
		VkDeviceSize WriteOffset = (VkDeviceSize)pCommand->m_pWriteOffset + WriteMemBlock.m_HeapData.m_Offset;
	...
	void Cmd_RenderText(const CCommandBuffer::SCommand_RenderText *pCommand)
	{
		...
		BindPipeline(CommandBuffer, PipeLine, pCommand->m_State);
		...
		vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PipeLayout, 0, 1, &DescrSet1.m_Descriptor, 0, nullptr);
	...
	void Cmd_RenderQuadContainerAsSpriteMultiple(const CCommandBuffer::SCommand_RenderQuadContainerAsSpriteMultiple *pCommand)
	{
		...
		while(DrawCount > 0)
		{
			...
			if(!CanBePushed)
			{
				...
				GetUniformBufferObject(UniDescrSet, UniformCount, (const float *)(pCommand->m_pRenderInfo + RenderOffset), UniformCount * sizeof(IGraphics::SRenderSpriteInfo));
	...
	void StartCommands(size_t CommandCount) override
	{
		if(IsVerbose())
		{
			dbg_msg("vulkan", "%zu commands in pipe.", CommandCount);
		}
	}
