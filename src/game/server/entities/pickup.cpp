...
CPickup::CPickup(CGameWorld *pGameWorld, int Type, int SubType, int Layer, int Number, int Flags) :
	CEntity(pGameWorld, EEntityClass::PICKUP, true, vec2(0, 0), PICKUP_PHYSICS_RADIUS)
{
...
void CPickup::Tick()
{
	...
	for(int i = 0; i < Num; ++i)
	{
		...
		if(pChr && pChr->IsAlive())
		{
			...
			case POWERUP_HEALTH:
				if(pChr->Freeze())
					Env()->CreateSound(m_Pos, SOUND_PICKUP_HEALTH, pChr->TeamMask(), pChr->GetCid());
				break;
			...
			case POWERUP_ARMOR:
				...
				if(Sound)
				{
					...
					Env()->CreateSound(m_Pos, SOUND_PICKUP_ARMOR, pChr->TeamMask(), pChr->GetCid());
				}
			...
			case POWERUP_WEAPON:
				...
				if(m_Subtype >= 0 && m_Subtype < NUM_WEAPONS && (!pChr->GetWeaponGot(m_Subtype) || pChr->GetWeaponAmmo(m_Subtype) != -1))
				{
					...
					if(m_Subtype == WEAPON_GRENADE)
						Env()->CreateSound(m_Pos, SOUND_PICKUP_GRENADE, pChr->TeamMask(), pChr->GetCid());
